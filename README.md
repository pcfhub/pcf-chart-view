...
[![Try it live on PCFHub](https://pcfhub.dev/badges/try-it-live.svg)](https://pcfhub.dev/components/pcf-chart-view)
...
| `parentLookup` | SingleLine.Text | — | On a subgrid, the lookup relating the rows to the form's record (`parentcustomerid`), or `none` for a subgrid that is not related. Blank: found from the table's relationships or the loaded rows |
